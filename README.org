...
*** Lexical Scoping
Since Emacs 31, dynamic scoping is basically deprecated, so this has to be at the start of any config file.
...
#+end_src
*** Behavior Fixes
These go first, so that even if loading the rest of this file fails, there's at least some fixes to the parts of Emacs which can largely be considered to be "bad defaults". At some point, I need to go back and move this into the ~emacs~ metapackage.
#+begin_src emacs-lisp
  ...
  (setopt read-process-output-max (* 16 1024 1024)) ;; 16MB
  ...
  ;; scroll horizontally conservatively
  ...
  ;; move point to the buffer boundary before signaling error
  ...
  ;; don't auto revert on remote buffers. so laggy
  ...
  (use-package tramp
    ...
    ;; stop polling all the damn time
  ...
  ;; kill dired buffers as we navigate
  (setopt dired-kill-when-opening-new-dired-buffer t)

  ;; use vc integration when renaming files in git
  (setopt dired-vc-rename-file t)

  ;; preset the margin size
  (setopt display-line-numbers-grow-only nil)
  (setopt display-line-numbers-width 3)
  (setopt display-line-numbers-width-start nil) 

  ;; make recursive deletes, copies, etc easier
  ...
  ;; hide free space thing
  (setopt dired-free-space nil)

  ;; only scroll dired files
  (setopt dired-movement-style 'bounded-files)

  ;; trigger dired reloads even faster, but not on tramp
  (setopt dired-auto-revert-buffer 'dired-directory-changed-p)
  (setopt dired-do-revert-buffer (lambda (dir)
...
#+begin_src emacs-lisp
  (use-package osx-trash
    ...
    (if (executable-find "trash")
      (setq-default osx-trash-command "trash"))
    (osx-trash-setup))
...
#+end_src
**** ~async~
Asynchronous compilation for Emacs. Note: this can sometimes really delay startup, and after startup there might still be Emacs processes running in the background, so be mindful of that if you're in a hurry.
#+begin_src emacs-lisp
  (use-package async
    :config
...
#+begin_src emacs-lisp
  ...
  (use-package solarized-theme
    :config
    (setopt solarized-high-contrast-mode-line nil
...
#+end_src
**** ~beacon~
Cool package that highlights where your cursor is on switched buffers.
#+begin_src emacs-lisp
  (use-package beacon
    :diminish
    :if window-system
...
#+end_src
**** ~goggles~
Flashes text as it's being added or removed. VERY slow in the terminal.
#+begin_src emacs-lisp
  (use-package goggles
    :diminish
    :if window-system
...
#+begin_src emacs-lisp
  (use-package comet-trail
    ...
    :config
    (setopt comet-trail-minimum-distance 50
...
#+begin_src emacs-lisp
  ...
  (setopt inhibit-startup-message t
        initial-scratch-message (concat
  			       (personal/prefix-comment
  				(cowsay-string "Emacs has finished starting." "tux")) "\n\n"))
#+end_src
**** ~org-superstar-mode~
...
#+end_src
**** ~mixed-pitch~
Makes text modes in Emacs prettier by using a non-monospaced font.
#+begin_src emacs-lisp
  (use-package mixed-pitch
    ...
    :config
    (setopt mixed-pitch-face 'variable-pitch))

  ;; the actual font used
  (set-face-attribute 'variable-pitch nil :family "IBM Plex Sans" :height 120)
#+end_src
We also want to sometimes bump a font size with ~text-scale-increase~, but if we're not smart, the minibuffer will weirdly own this and think it belongs to it. To fix this:
#+begin_src emacs-lisp
  (defun personal/prose-scale ()
    (text-scale-set 1.25)
    (add-hook 'minibuffer-setup-hook #'personal/reset-minibuffer-scale nil t))

  (defun personal/reset-minibuffer-scale ()
    (text-scale-set 0))

  (add-hook 'markdown-mode-hook #'personal/prose-scale)
...
#+begin_src emacs-lisp
  (use-package eat
    ...
    :config
    ;; perf boosting
    (setopt eat-minimum-latency 0.01
...
#+begin_src emacs-lisp
  (use-package vterm
    ...
    (setopt vterm-color-black   "#000000"
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    ;; async speed improvements
    (setopt consult-async-input-debounce 0.05
...
#+begin_src emacs-lisp
  (use-package cape
    :config
    (setopt cape-dict-file "/usr/share/dict/words")
...
#+begin_src emacs-lisp
  (use-package pdf-view-restore
    ...
    :diminish
    :config
    (setopt pdf-view-restore-filename "~/.emacs.d/pdf-view-restore")
...
#+begin_src emacs-lisp
  (use-package vundo
    :config
    (setopt diff-switches "-u --color=never"
...
#+begin_src emacs-lisp
  (use-package gptel
    :config
    (setopt gptel-model 'gemma4:e4b-mlx
...
#+begin_src emacs-lisp
  (use-package mindstream
    ;; TODO: make this not spam directory creation at init time
    :disabled t
    :config
    ;; save to $HOME/Documents/Scratch
    (setopt mindstream--user-home-directory (expand-file-name "~/Documents/Scratch"))
    ;; must be set later, otherwise it creates at the wrong path
    (setopt mindstream-persist t)
    (mindstream-mode))
#+end_src
