...
It works best on macOS and in a graphical frame. For the best experience, use Yamamoto Mitsuharu's Emacs macOS port, or a pureGTK build on Linux. Currently, it only works on Emacs versions newer than 28 (and given the current version is Emacs 30, it's unlikely I'll ever go back and add support for older versions).
...
- Make Emacs faster since it's kind of extremely slow now, especially at bootup
...
#+end_src
**** ~mac-pseudo-daemon~
This does a neat trick where we get Emacs daemon-like functionality on macOS. Very nice!
#+begin_src emacs-lisp
  (use-package mac-pseduo-daemon
    :disabled t
    :ensure nil
    :vc (mac-pseudo-daemon :url "https://github.com/DarwinAwardWinner/mac-pseudo-daemon"
			   :branch "main")
    :init
...
#+begin_src emacs-lisp
  (use-package cape
    :ensure t
    :hook ((fundamental-mode) . personal/cape-dict) ; only run when needed, it's expensive
    :init
    (add-hook 'completion-at-point-functions #'cape-dabbrev)
    (add-hook 'completion-at-point-functions #'cape-file)
    (add-hook 'completion-at-point-functions #'cape-history))

  ;; TODO: it would be sick if we could somehow have hovers pull up the dictionary definition
  (defun personal/cape-dict ()
...
The default and it's pretty good. Miles ahead of LSP mode but we still need better debug tools.
#+begin_src emacs-lisp
  (use-package eglot
    :ensure t
...
#+end_src
The booster also helps, because I always bring the Rust toolchain with me:
#+begin_src emacs-lisp
